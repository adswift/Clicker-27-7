...
class ScoreTableViewController: UITableViewController {
    ...
    @IBAction func unwindToScoreTable(segue: UIStoryboardSegue){
        if segue.identifier == "exitClicker"{
            ...
            scores.append(Double(source.time))
    ...
    @IBAction func unwindToScore (segue: UIStoryboardSegue){
        if segue.identifier == "exitClicker", let source = segue.source as? clickerViewController {
            let score = source.time
            let scoreAsDouble = Double(round(score * 10)) / 10
